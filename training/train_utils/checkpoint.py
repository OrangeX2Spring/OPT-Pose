...
def robust_torch_save(checkpoint: Dict[str, Any], checkpoint_path: str) -> None:
    ...
    # If a backup file already exists (from a previous interrupted save), remove it
    if g_pathmgr.exists(backup_checkpoint_path):
        logging.warning(f"Found existing backup file {backup_checkpoint_path}, removing it...")
