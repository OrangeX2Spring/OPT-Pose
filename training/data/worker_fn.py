...
def default_worker_init_fn(worker_id, num_workers, epoch, seed=0):
    ...
    # Safely parse distributed rank from environment if present, default to 0
    _rank_env = os.environ.get("RANK", None)
    distributed_rank = int(_rank_env) if _rank_env is not None else 0
