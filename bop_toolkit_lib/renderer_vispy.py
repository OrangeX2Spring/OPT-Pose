...
# CRITICAL: Import OpenGL initialization FIRST before any other imports
try:
    from . import __init_opengl__
except ImportError:
...
import vispy
from . import inout, misc, renderer
from vispy import app, gloo

# app backends: egl, glfw, pyglet, 
# gl backends: gl2, pyopengl2, gl+
app_backend = "egl"
# Use 'gl2' backend instead of 'pyopengl2' to avoid bufSize issues
gl_backend = "gl2"  # Changed from "pyopengl2" to avoid OpenGL_accelerate issues
vispy.use(app=app_backend, gl=gl_backend)
print("vispy uses app: {}, gl: {}".format(app_backend, gl_backend))

# Safe wrapper for glReadPixels to avoid bufSize issues
def safe_glReadPixels(x, y, width, height, format, type, array):
    """Wrapper for glReadPixels that handles both accelerated and non-accelerated versions."""
    try:
        # Try calling without bufSize (standard PyOpenGL)
        gl.glReadPixels(x, y, width, height, format, type, array)
    except TypeError as e:
        if 'bufSize' in str(e):
            # If bufSize error, try with explicit bufSize parameter
            try:
                gl.glReadPixels(x, y, width, height, format, type, array, array.nbytes)
            except:
                # Fallback: use ctypes approach
                from ctypes import c_void_p
                gl.glReadPixels(x, y, width, height, format, type, c_void_p(array.ctypes.data))
        else:
            raise
...
class RendererVispy(renderer.Renderer, app.Canvas, metaclass=SingletonArgs):
  ...
  def _draw_rgb(self, obj_id, mat_model, mat_view, mat_proj):
    ...
    safe_glReadPixels(0, 0, self.width, self.height, gl.GL_RGB, gl.GL_UNSIGNED_BYTE, rgb)
    rgb = rgb[::-1, :]
    return rgb
  ...
  def _draw_depth(self, obj_id, mat_model, mat_view, mat_proj):
    ...
    dep = np.empty((self.height, self.width), dtype=np.float32)
    safe_glReadPixels(0, 0, self.width, self.height, gl.GL_DEPTH_COMPONENT, gl.GL_FLOAT, dep)
    dep = dep[::-1, :]
