...
def mssd(R_est, t_est, R_gt, t_gt, pts, syms):
  ...
  pts_est = misc.transform_pts_Rt(pts.astype(np.float16), R_est, t_est)
  es = []
  for sym in syms:
    R_gt_sym = R_gt.dot(sym['R']).astype(np.float16)
    t_gt_sym = (R_gt.dot(sym['t']) + t_gt).astype(np.float16)
...
def mspd(R_est, t_est, R_gt, t_gt, K, pts, syms):
  ...
  R_est, t_est, pts, K = R_est.astype(np.float16), t_est.astype(np.float16), pts.astype(np.float16), K.astype(np.float16)
...
def my_mssd(R_est: np.ndarray, t_est: np.ndarray, R_gt: np.ndarray, t_gt: np.ndarray, pts: np.ndarray, syms: np.ndarray):
  """Maximum Symmetry-Aware Surface Distance (MSSD).
...
def my_mspd(R_est: np.ndarray, t_est: np.ndarray, R_gt: np.ndarray, t_gt: np.ndarray, K: np.ndarray, pts: np.ndarray, syms: np.ndarray):
